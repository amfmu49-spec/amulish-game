...
`).map(e=>e.trim()).filter(e=>e.length>0&&!/^\[.*\]$/.test(e)&&!/^\(.*\)$/.test(e)).map((e,t)=>({id:`fb_`+t,time:500+t*1800,end:3e3+t*1800,text:e}));e.length>0&&(y.setLyrics(e),A=`フォールバック歌詞: ${e.length}行`,console.log(`[AMULISH] Using fallback lines:`,e.slice(0,5).map(e=>e.text)))}if(w){let e=T.split(/[\s,._\-／/]+/).filter(e=>e.length>1).filter(e=>!t(e));y.setCustomWords(e)}B(),A&&setTimeout(()=>{let e=document.getElementById(`suno-status-box`),t=document.querySelector(`.suno-status-title`),n=document.getElementById(`suno-track-name`);e&&(e.style.display=`flex`),t&&(k.length>0?(t.textContent=`✅ 歌詞セット完了！`,t.style.color=`#00ff66`):(t.textContent=`⚠️ 歌詞取得できず（デモ語で表示）`,t.style.color=`#ff6622`)),n&&(n.textContent=A)},100),w&&v.loadMusic(w,e=>{V(e)}),v.setOnTime(e=>y.syncTime(e));var j=0;function M(e){let t=Math.min(e-j,50);j=e,y.phase===`PLAYING`?(y.update(t),b.render(y,h,g)):y.phase===`TITLE`?b.renderTitle(y,h,g):y.phase===`RESULT`&&b.renderResult(y,h,g),requestAnimationFrame(M)}requestAnimationFrame(M);var N=0,P=0,F=!1;function I(e,t){N=e,P=t,F=!0,y.phase!==`PLAYING`&&(v.resume(),y.startGame(),H())}function L(e,t){F&&(y.movePlayer(e-N,t-P),N=e,P=t)}function R(){F=!1}p.addEventListener(`touchstart`,e=>{e.preventDefault(),I(e.changedTouches[0].clientX,e.changedTouches[0].clientY)},{passive:!1}),p.addEventListener(`touchmove`,e=>{e.preventDefault(),L(e.changedTouches[0].clientX,e.changedTouches[0].clientY)},{passive:!1}),p.addEventListener(`touchend`,R),p.addEventListener(`mousedown`,e=>I(e.clientX,e.clientY)),p.addEventListener(`mousemove`,e=>L(e.clientX,e.clientY)),p.addEventListener(`mouseup`,R);var z=`javascript:(function(){var s='',a=document.querySelector('audio');if(a&&a.src&&!a.src.startsWith('blob:'))s=a.src;var m=(location.href+'|'+(a&&a.src||'')).match(/([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/i);if(m)s='https://cdn1.suno.ai/'+m[1]+'.mp3';if(!s){alert('SUNOの曲ページで実行してね');return;}var L='',e=document.querySelector('[data-testid="song-lyrics"]');if(e)L=e.innerText||'';if(!L){var found=[];[].forEach.call(document.querySelectorAll('div,p'),function(d){var t=d.childElementCount<5?d.innerText||'':'';var n=t.split('\\n').filter(function(x){return x.trim();}).length;if(n>=4&&t.length>50&&t.length<4000)found.push(t);});if(found.length)L=found.reduce(function(a,b){return b.length>a.length?b:a;});}var t=(document.title||'').replace(/\\s*[|\\-].*$/,'').trim()||'SUNO';var u='https://amfmu49-spec.github.io/amulish-game/?audio='+encodeURIComponent(s)+'&title='+encodeURIComponent(t);if(L)u+='&lyrics='+encodeURIComponent(L.substring(0,3000));window.open(u,'_blank');})();`;function B(){let e=document.createElement(`div`);if(e.id=`ui-overlay`,e.innerHTML=`
    ...
    <div id="ui-panel">
      ...
      <div class="suno-status-box" id="suno-status-box">
        ...
        <div class="suno-status-track" id="suno-track-name">${T}</div>
  ...
  `,document.body.appendChild(e),document.getElementById(`bm-copy`).addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(z);let e=document.getElementById(`bm-copy`);e.textContent=`✅ コピー完了！ブックマークに保存してね`,e.classList.add(`bm-copied`),setTimeout(()=>{e.textContent=`📋 ブックマークレットをコピー`,e.classList.remove(`bm-copied`)},3e3)}catch{prompt(`コピーしてブックマークに登録してください:`,z)}}),document.querySelectorAll(`.diff-btn`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.diff-btn`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),y.setDifficulty(e.dataset.diff)})}),document.getElementById(`btn-start`).addEventListener(`click`,()=>{v.resume(),y.startGame(),H()}),document.getElementById(`btn-retry`).addEventListener(`click`,()=>{v.seek(0),y.startGame(),H()}),document.getElementById(`btn-to-title`).addEventListener(`click`,()=>{v.pause(),y.goToTitle(),U()}),w){let e=document.getElementById(`suno-status-box`);e.style.display=`flex`}}function V(e){let t=document.getElementById(`progress-bar-fill`),n=document.getElementById(`progress-text`),r=document.querySelector(`.suno-status-title`);if(e===-1){t&&(t.style.width=`100%`),t&&(t.style.background=`#ff3355`),n&&(n.textContent=`準備完了`),r&&(r.textContent=`🎵 SUNO曲セット完了 («START»で再生)`,r.style.color=`#ffe600`);return}let i=Math.max(0,Math.min(100,e));t&&(t.style.width=Math.max(4,i)+`%`),n&&(n.textContent=i>0?i+`%`:`読込中... (STARTで起動)`),i>=100&&r&&(r.textContent=`✅ SUNO曲の読み込み完了！`,r.style.color=`#00ff66`)}function H(){document.getElementById(`ui-panel`).style.display=`none`,document.getElementById(`result-panel`).style.display=`none`}function U(){document.getElementById(`ui-panel`).style.display=`flex`,document.getElementById(`result-panel`).style.display=`none`}function W(){let e={S:`#ffe600`,A:`#00ff66`,B:`#00f3ff`,C:`#ff6622`};document.getElementById(`result-rank`).textContent=y.rank,document.getElementById(`result-rank`).style.color=e[y.rank],document.getElementById(`r-score`).textContent=y.score.toLocaleString(),document.getElementById(`r-combo`).textContent=y.maxCombo+`x`,document.getElementById(`r-acc`).textContent=y.accuracy+`%`,document.getElementById(`r-diff`).textContent=y.difficulty,document.getElementById(`ui-panel`).style.display=`none`,document.getElementById(`result-panel`).style.display=`flex`}window.__showResult=W;