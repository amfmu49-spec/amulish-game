...
`).map(e=>e.trim()).filter(e=>e.length>0&&!/^\[.*\]$/.test(e)&&!/^\(.*\)$/.test(e)).map((e,t)=>({id:`fb_`+t,time:500+t*1800,end:3e3+t*1800,text:e}));e.length>0&&(v.setLyrics(e),N=`フォールバック歌詞: ${e.length}行`,console.log(`[AMULISH] Using fallback lines:`,e.slice(0,5).map(e=>e.text)))}if(D){let e=O.split(/[\s,._\-／/]+/).filter(e=>e.length>1).filter(e=>!t(e));v.setCustomWords(e)}G(),N&&setTimeout(()=>{let e=document.getElementById(`suno-status-box`),t=document.querySelector(`.suno-status-title`),n=document.getElementById(`suno-track-name`);e&&(e.style.display=`flex`),t&&(M.length>0?(t.textContent=`✅ 歌詞セット完了！`,t.style.color=`#00ff66`):(t.textContent=`⚠️ 歌詞取得できず（デモ語で表示）`,t.style.color=`#ff6622`)),n&&(n.textContent=N)},100),D&&_.loadMusic(D,e=>{K(e)}),_.setOnTime(e=>v.syncTime(e));var P=0,F=`TITLE`;function I(e){let t=Math.min(e-P,50);P=e,v.phase===`PLAYING`?(v.update(t),y.render(v,m,h)):v.phase===`TITLE`?y.renderTitle(v,m,h):v.phase===`RESULT`&&(F!==`RESULT`&&L(),y.renderResult(v,m,h)),F=v.phase,requestAnimationFrame(I)}requestAnimationFrame(I);function L(){let e={S:`#ffe600`,A:`#00ff66`,B:`#00f3ff`,C:`#ff6622`},t=document.querySelector(`#result-panel h2`),n=v.playerHp<=0;t&&(n?(t.textContent=`💀 GAME OVER`,t.style.color=`#ff2255`,t.style.textShadow=`0 0 20px #ff2255`):(t.textContent=`🎉 STAGE CLEAR`,t.style.color=`#ffe600`,t.style.textShadow=`0 0 20px #ffe600`)),document.getElementById(`result-rank`).textContent=n?`FAILED`:v.rank,document.getElementById(`result-rank`).style.color=n?`#ff2255`:e[v.rank],document.getElementById(`r-score`).textContent=v.score.toLocaleString(),document.getElementById(`r-combo`).textContent=v.maxCombo+`x`,document.getElementById(`r-acc`).textContent=v.accuracy+`%`,document.getElementById(`r-diff`).textContent=v.difficulty,document.getElementById(`ui-panel`).style.display=`none`,document.getElementById(`result-panel`).style.display=`flex`}var R=0,z=0,B=!1;function V(e,t){R=e,z=t,B=!0,v.phase!==`PLAYING`&&(_.resume(),v.startGame(),q())}function H(e,t){B&&(v.movePlayer(e-R,t-z),R=e,z=t)}function U(){B=!1}f.addEventListener(`touchstart`,e=>{e.preventDefault(),V(e.changedTouches[0].clientX,e.changedTouches[0].clientY)},{passive:!1}),f.addEventListener(`touchmove`,e=>{e.preventDefault(),H(e.changedTouches[0].clientX,e.changedTouches[0].clientY)},{passive:!1}),f.addEventListener(`touchend`,U),f.addEventListener(`mousedown`,e=>V(e.clientX,e.clientY)),f.addEventListener(`mousemove`,e=>H(e.clientX,e.clientY)),f.addEventListener(`mouseup`,U);var W=`javascript:(function(){var s=document.createElement('script');s.src='https://amfmu49-spec.github.io/amulish-game/bookmarklet.js?t='+Date.now();document.body.appendChild(s);})();`;function G(){let e=document.createElement(`div`);if(e.id=`ui-overlay`,e.innerHTML=`
    ...
    <div id="ui-panel">
      ...
      <div class="suno-status-box" id="suno-status-box">
        ...
        <div class="suno-status-track" id="suno-track-name">${O}</div>
  ...
  `,document.body.appendChild(e),document.getElementById(`bm-copy`).addEventListener(`click`,async()=>{try{await navigator.clipboard.writeText(W);let e=document.getElementById(`bm-copy`);e.textContent=`✅ コピー完了！ブックマークに保存してね`,e.classList.add(`bm-copied`),setTimeout(()=>{e.textContent=`📋 ブックマークレットをコピー`,e.classList.remove(`bm-copied`)},3e3)}catch{prompt(`コピーしてブックマークに登録してください:`,W)}}),document.querySelectorAll(`.diff-btn`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.diff-btn`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),v.setDifficulty(e.dataset.diff)})}),document.getElementById(`btn-start`).addEventListener(`click`,()=>{_.resume(),v.startGame(),q()}),document.getElementById(`btn-retry`).addEventListener(`click`,()=>{_.seek(0),v.startGame(),q()}),document.getElementById(`btn-to-title`).addEventListener(`click`,()=>{_.pause(),v.goToTitle(),J()}),D){let e=document.getElementById(`suno-status-box`);e.style.display=`flex`}}function K(e){let t=document.getElementById(`progress-bar-fill`),n=document.getElementById(`progress-text`),r=document.querySelector(`.suno-status-title`);if(e===-1){t&&(t.style.width=`100%`),t&&(t.style.background=`#ff3355`),n&&(n.textContent=`準備完了`),r&&(r.textContent=`🎵 SUNO曲セット完了 («START»で再生)`,r.style.color=`#ffe600`);return}let i=Math.max(0,Math.min(100,e));t&&(t.style.width=Math.max(4,i)+`%`),n&&(n.textContent=i>0?i+`%`:`読込中... (STARTで起動)`),i>=100&&r&&(r.textContent=`✅ SUNO曲の読み込み完了！`,r.style.color=`#00ff66`)}function q(){document.getElementById(`ui-panel`).style.display=`none`,document.getElementById(`result-panel`).style.display=`none`}function J(){document.getElementById(`ui-panel`).style.display=`flex`,document.getElementById(`result-panel`).style.display=`none`}